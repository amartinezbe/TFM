...
datos1 <- datos1[!is.na(datos1$q4_2),]
...
if (val == "GLOBAL")
  ...
  jpeg(file = paste("binomial",val,as.character(fl),".jpg", sep=""),width = 800, height = 600)
  #densidad por violin y agrupado pais
  ggplot(datosp, aes(x = q4_2BIN, y = d1)) +
  ...
  fl=fl+1
  jpeg(file = paste("binomial",val,as.character(fl),".jpg", sep=""),width = 800, height = 600)
  #densidad por violin y agrupado sexo
  ggplot(datosp, aes(x = q4_2BIN, y = d1)) +
    geom_violin(fill = "lightblue") +
    facet_wrap(~ d2) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1))
  ...
  fl=fl+1
  jpeg(file = paste("binomial",val,as.character(fl),".jpg", sep=""),width = 800, height = 600)
  #histograma edad por sexo
  ggplot(datosp, aes(x=d1, color=d2)) +
    geom_histogram(binwidth=8, fill="white", alpha=0.5, position="identity") +
    theme(axis.text.x = element_text(angle = 90, hjust = 1))
  dev.off()
  ...
  fl=fl+1
  jpeg(file = paste("binomial",val,as.character(fl),".jpg", sep=""),width = 800, height = 600)
  #barchar por paises de la variable q4_2BIN
  ggplot(datosp, aes(x = q4_2BIN, color=d2)) +
    geom_bar(fill="lightblue",alpha=0.5, position="identity") +
    facet_wrap(~ isocntry) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1))
  dev.off()
  ...
  fl=fl+1
  jpeg(file = paste("binomial",val,as.character(fl),".jpg", sep=""),width = 800, height = 600)
  #barchar por paises de la variable q2 veces que encuentro
  ggplot(datosp, aes(x = q2, color=d2)) +
    geom_bar(fill="lightblue",alpha=0.5, position="identity") +
    facet_wrap(~ isocntry) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1))
  dev.off()
  ...
  fl=fl+1
  jpeg(file = paste("binomial",val,as.character(fl),".jpg", sep=""),width = 800, height = 600)
  #barchar por paises de la variable d4r2 (edad que se finalizan los estudios)
  ggplot(datosp, aes(x = d4r2, color=d2)) +
    geom_bar(fill="lightblue",alpha=0.5, position="identity") +
    facet_wrap(~ isocntry) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1))
  dev.off()
  ...
  fl=fl+1
  jpeg(file = paste("binomial",val,as.character(fl),".jpg", sep=""),width = 800, height = 600)
  #importante. se ve claramente que q2 afecta a la percepción de problema
  ggplot(datosp) + 
    geom_mosaic(aes(x=product(q4_2BIN, q2),fill=q4_2BIN)) +
    facet_wrap(~ isocntry) +
    theme(axis.text.x = element_text(angle = 90, hjust = 1))
  dev.off()
  
  sink(file=(paste("polr",val,".txt", sep="")))  
...
#En el punto siguiente estudiaremos la selección de variables para quedarnos con 
  ...
  stargazer(mbg1, mfinal, title=paste("Pa?s: ",val,".  Resultados modelos inicial y final", sep=""),
           align=TRUE, type="text",
           dep.var.labels=c("Percepci?n afectaci?n democracia"),
...
#ha eliminado variables?
 # print(val)
  #print("Estad?sticos multivariante Modelo inicial y modelo final")
  print(blr_multi_model_fit_stats(mbg1, mfinal))
  print(" ")
  print ("odd ratios")
    Cbind(0, Estimate=round(coef(mfinal),4), OR=round(exp(coef(mfinal)),4))
  
  
#  Modelo final
  print(coeftest(mfinal))   
  print(Anova(mfinal))
  print(summary(mfinal)) # display results
  print(confint(mfinal)) # 95% CI for the coefficients
  print(exp(coef(mfinal))) # exponentiated coefficients
  print(exp(confint(mfinal))) # 95% CI for exponentiated coefficients
